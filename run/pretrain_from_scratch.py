...
def main(opts):
    ...
    if opts.checkpoint:
        ...
        checkpoint = {}
    model = VLFromScratch.from_pretrained(
        opts.model_config, checkpoint, img_dim=opts.img_dim, img_label_dim=opts.img_label_dim,
...
def validate(model, val_dataloaders, img_dim):
    model.eval()
    for task, loader in val_dataloaders.items():
        ...
        if task.startswith('mlm'):
            ...
            val_log = validate_mrfr(model, loader, img_dim)
        elif task.startswith('itm'):
...
@torch.no_grad()
def validate_mrfr(model, val_loader, img_dim):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_label_dim', type=int, default=IMG_LABEL_DIM)
    ...
    parser.add_argument('--gradient_accumulation_steps', type=int, default=16,
                        help="Num updates steps to accumulate before a backward/update pass.")
